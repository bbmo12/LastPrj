...
public class Scheduler {
	 ...
	 * 
	 * // 현재 시간 LocalTime time = LocalTime.now(); // 포맷 정의하기 DateTimeFormatter
	 * formatter = DateTimeFormatter.ofPattern("HH시"); // 포맷 적용하기 String formatedNow
	 * = time.format(formatter); // 포맷 적용된 현재 시간 출력
	 * System.out.println("현재시간 : "+formatedNow); // 06시 20분 57초
	 * 
	 * LocalDate now = LocalDate.now(); // 연도, 월(문자열, 숫자), 일, 일(year 기준), 요일(문자열,
	 * 숫자) int year = now.getYear(); String month = now.getMonth().toString(); int
	 * monthValue = now.getMonthValue(); int dayOfMonth = now.getDayOfMonth(); int
	 * dayOfYear = now.getDayOfYear(); System.out.println("현재날짜 : "+now);
	 * 
	 * }
	 */
